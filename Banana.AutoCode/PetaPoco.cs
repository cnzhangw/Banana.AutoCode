...
namespace Banana
{
    ...
    public class Database : IDisposable
    {
        ...
        #region extenions by zhangw


        internal string[] GetColumns<T>(Expression<Func<T, object>> expression)
        {
            ...
            {
                foreach (var item in ((NewExpression)expression.Body).Arguments)
                {
                    var member = (item as MemberExpression).Member;
                    object[] ats = member.GetCustomAttributes(typeof(ColumnAttribute), false);
                    if (ats.Length > 0)
                    {
                        list.Add(((ColumnAttribute)ats[0]).Name);
                    }
                    else
                    {
        ...
        internal void Fill(DataTable dt, string sql, params object[] args)
        {
            OpenSharedConnection();
            try
            {
    ...
    */
    ...
    public class Sql
    {
        ...
        {
            if (columns.Length == 0 || string.IsNullOrWhiteSpace(columns[0].ToString()))
            {
                return this;
            }
    ...
    namespace DatabaseTypes
    {
        ...
        class OracleDatabaseType : DatabaseType
        {
            ...
            {
                var pi = cmd.GetType().GetProperty("BindByName", BindingFlags.NonPublic | BindingFlags.Instance);
                if (pi != null)
                {
